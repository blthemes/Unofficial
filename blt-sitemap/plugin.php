...
class pluginBltSitemap extends Plugin {
	...
	public function form()
	{
		...
		$html .= '<a href="'.DOMAIN_BASE.'sitemap.xml'.'">'.DOMAIN_BASE.'sitemap.xml'.'</a>';
		$html .= '</div>';
	...
	private function createXML()
	{
		...
		$xml .= '<?xml-stylesheet type="text/xsl" href="'.DOMAIN_BASE.'sitemap.xsl"?>';
	...
	{
		$diff = time() - $this->getValue('lastPing');
		if($diff > 60*60){ //1hour - prevent continuous pings
			if ($this->getValue('pingGoogle')) {
				
				$url = 'https://www.google.com/webmasters/sitemaps/ping?sitemap=' . htmlentities(DOMAIN_BASE.'sitemap.xml');
				TCP::http($url, 'GET', false, 3);
				//added url from official Google docs
				$url ='https://www.google.com/ping?sitemap=' . htmlentities(DOMAIN_BASE.'sitemap.xml');
				TCP::http($url, 'GET', false, 3);
			}

			if ($this->getValue('pingBing')) {
				$url = 'https://www.bing.com/webmaster/ping.aspx?sitemap=' . htmlentities(DOMAIN_BASE.'sitemap.xml');
				TCP::http($url, 'GET', false, 3);
			}
			$this->setField('lastPing', time());
		}
	}
	...
	public function beforeAll()
	{
		...
		if( $this->webhook('sitemap.xsl') ) {
			
			header('Content-type: text/xml');
			include($this->phpPath . 'sitemap-xsl.php');
			exit(0);

...
class pluginBltRSSFeed extends Plugin
{
    ...
    private function get_excerpt($content, $maxwords = 100, $more = '...')
    {
        ...
        $excerpt = Sanitize::html($excerpt);

        return $excerpt;
    }

    private function imagefile_mime_type($filename)
    {
        $mime_types = array(
			'png' => 'image/png',
			'jpe' => 'image/jpeg',
    ...
    private function createXML()
    {
        ...
        $xml .= '
        <atom:link href="'.DOMAIN_BASE.'feed" rel="self" type="application/rss+xml" />';
        $xml .= '
        <link>'.$site->url().'</link>';
        ...
        foreach ($list as $pageKey) {
            try {              
                ...
                $xml .= '<description>'.$this->get_excerpt($page->contentBreak(), $maxWords).'</description>';
